...
object TrainSGDColdStart {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length != 2) {
      println("Usage: TrainSGDColdStart <input_path> <output_path>")
      sys.exit(1)
    }
    ...
    val spark = SparkSession.builder()
      .appName("BookSGD-Trainer-NonLoggedIn")
    ...
    try {
      ...
      userFactorsLike.write.mode("overwrite").parquet(s"$outputPath/non_logged_in/userFactorsLike")
      itemFactorsLike.write.mode("overwrite").parquet(s"$outputPath/non_logged_in/itemFactorsLike")
      userFactorsClick.write.mode("overwrite").parquet(s"$outputPath/non_logged_in/userFactorsClick")
      itemFactorsClick.write.mode("overwrite").parquet(s"$outputPath/non_logged_in/itemFactorsClick")
      ...
      userContentWeights.write.mode("overwrite").parquet(s"$outputPath/non_logged_in/userContentWeights")

      println("Training completed successfully")

    } catch {
      case e: Exception =>
        println(s"Error in TrainSGDColdStart: ${e.getMessage}")
        e.printStackTrace()
        sys.exit(1)
    } finally {
      spark.stop()
